    ...
    .then(data => {
        ...
        if (chatElement) {
            ...
            if (avatarImg && avatarImg.src) {
                avatarHtml = `<img src="${avatarImg.src}" class="talker">`;
            }
        }
        ...
        let upper_chatHtml = `
            <div class="upper_chat bg">
                ...
                <div class="right">
                    <button onclick="on_blur(); showProfile(${senderId});" class="small" style="margin-right: 8px;">
                    ...
                    <button class="end small" style="margin-right: 10px;" onclick="startVideoCall()">
        ...
        let send_message_fieldHtml = `
            <div class="send_message_field bg">
                <form class="message-form" data-chat-id="${chatId}">
                    <button class="btn pin" style="margin-right: 10px;" onclick="showContextMenuPin();">
                    ...
                    <input class="send_message_input" type="text" name="message" placeholder="Введите сообщение...">
        ...
        if (data.messages && data.messages.length > 0) {
            ...
            data.messages.forEach(message => {
                ...
                const msgAvatarImg = msgAvatarUrl ? `<span class="blur_loader"></span><img src="${msgAvatarUrl}" class="talker">` : '<span class="blur_loader"></span>';
                const msgSenderName = message.is_own ? 'Вы' : message.sender;
                
                let imagesHtml = '';
                if (message.images && message.images.length > 0) {
                    message.images.forEach(img => {
                        imagesHtml += `<div class="message_image_container"><span class="blur_loader"></span><img src="${img}" onclick="on_blur(); openImage('${img}');"></div>`;
                    });
                }
