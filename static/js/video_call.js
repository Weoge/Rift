class VideoCall {
    ...
    async start(chatId, isInitiator, isVideo) {
        ...
        if (remoteVideoContainer) remoteVideoContainer.classList.add('calling');
        ...
        if (isVideo) {
            if (videoContainer) videoContainer.style.flexDirection = 'row';
        ...
        if (toggleVideoBtn) toggleVideoBtn.style.display = isVideo ? 'block' : 'none';
        
        const remoteInfoEl = document.getElementById('remoteVideoInfo');
        const localInfoEl = document.getElementById('localVideoInfo');
        if (remoteInfoEl && localInfoEl) {
            const selectedChat = document.querySelector('.chat.selected');
            const remoteName = selectedChat ? selectedChat.querySelector('.talker_name b').textContent : '';
            remoteInfoEl.textContent = remoteName;
            const userNameEl = document.querySelector('.user_info h1');
            localInfoEl.textContent = userNameEl ? userNameEl.textContent : '';
            
            const remoteAvatar = document.querySelector('.remoteVideoContainer .avatar_container img');
            const talkerAvatar = selectedChat ? selectedChat.querySelector('.avatar_container img') : null;
            if (remoteAvatar && talkerAvatar) {
                remoteAvatar.src = talkerAvatar.src;
            }
        }
        ...
        try {
            ...
            this.websocket.onopen = function() {
                ...
                self.websocket.send(JSON.stringify({ type: 'audio_state', isMuted: false }));
                if (isVideo) self.websocket.send(JSON.stringify({ type: 'video_state', isVideoMuted: false }));
    ...
    createPeerConnection() {
        ...
        this.peerConnection.ontrack = function(event) {
            ...
            if (remoteVideoContainer) remoteVideoContainer.classList.remove('calling');
            document.getElementById('callStatus').textContent = `${gettext("Подключено")}`;
            self.startCallTimer();
    ...
    startSpeakerDetection() {
        ...
        if (this.peerConnection) {
            this.peerConnection.getReceivers().forEach(receiver => {
                if (receiver.track && receiver.track.kind === 'audio') {
                    const remoteSource = this.audioContext.createMediaStreamSource(new MediaStream([receiver.track]));
        ...
        this.speakerCheckInterval = setInterval(checkAudio, 100);
    }
    ...
    async handleSignal(data) {
        ...
        if (data.type === 'audio_state') {
            const remoteVideo = document.getElementById('remoteVideo');
            const microStatus = remoteVideo ? remoteVideo.parentElement.querySelector('.micro_status') : null;
            if (microStatus) {
                if (data.isMuted) {
                    microStatus.classList.add('muted');
                    microStatus.style.display = 'block';
                } else {
                    microStatus.classList.remove('muted');
            ...
            if (remoteVideo) {
                if (data.isVideoMuted) {
                    remoteVideo.classList.add('video-disabled');
                } else {
                    remoteVideo.classList.remove('video-disabled');
                }
            }
            ...
            if (this.isInitiator) {
                ...
                this.websocket.send(JSON.stringify({ type: 'offer', offer: offer, isVideo: this.isVideo }));
            }
        } else if (data.type === 'offer') {
            console.log('Received offer, creating answer...');
            if (data.isVideo !== undefined && data.isVideo !== this.isVideo) {
    ...
    toggleAudio() {
        const toggleAudio = document.getElementById('toggleAudio');
        const localVideo = document.getElementById('localVideo');
        const microStatus = localVideo ? localVideo.parentElement.querySelector('.micro_status') : null;
        if (this.localStream) {
            ...
            if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
                this.websocket.send(JSON.stringify({ type: 'audio_state', isMuted: this.isAudioMuted }));
            }
        }
    }
    ...
    toggleVideo() {
        ...
        if (this.localStream && videoTracks.length > 0) {
            ...
            toggleVideo.classList.toggle('muted', this.isVideoMuted);
            if (localVideo) {
                if (this.isVideoMuted) {
                    localVideo.classList.add('video-disabled');
                } else {
                    localVideo.classList.remove('video-disabled');
                }
            }
            ...
            if (this.websocket && this.websocket.readyState === WebSocket.OPEN) {
                this.websocket.send(JSON.stringify({ type: 'video_state', isVideoMuted: this.isVideoMuted }));
            }
        }
    }
        ...
        if (this.speakerCheckInterval) clearInterval(this.speakerCheckInterval);
        ...
        document.querySelector('.localVideoContainer').classList.remove('speaking');
        document.querySelector('.remoteVideoContainer').classList.remove('speaking');
        ...
        fetch('/app/call/' + chatId + '/initiate/')
            ...
            .then(function(data) {
                if (data.status === 'success') {
                    videoCall.start(chatId, true, true);
                }
            });
    }
}
...
function startAudioCall() {
    ...
    if (selectedChat) {
        ...
        fetch('/app/call/' + chatId + '/initiate/')
            ...
            .then(function(data) {
                if (data.status === 'success') {
                    videoCall.start(chatId, true, false);
                }
            });
    }
}

function answerCall(chatId, isVideo) {
    if (isVideo === undefined) isVideo = true;
    videoCall.start(chatId, false, isVideo);
}
