...
function showProfile(user_id) {
    ...
    let modal_up_new = `
        ...
        <span class="title font-bold">${gettext("Профиль")}</span>
    `;
        ...
        .then(data => {
            ...
            profileHtml = `
                <div class="profile">
                    ...
                    <p style="color: #fff;">${gettext("Был(а)")} ${data.last_login}</p>
                    <p style="color: #fff;">${gettext("Дата регистрации:")} ${data.date_joined}</p>
                </div>
            `;
            showCustomModal(gettext("Профиль"), profileHtml);
        })
        .catch(error => {
...
function closeModal() {
    ...
    if (modal_title == `${gettext("Профиль")}`) {
        showProfileDataBack();
    }
}
